...
**And** per **ciascuna** Prenotazione coinvolta il Conflitto **espone** fonte (il Canale) e timestamp di sincronizzazione, **derivati alla lettura** dallo stato corrente della Prenotazione e della sua sorgente: il valore che l'Host vede è quello vero **nel momento in cui guarda**, mai una fotografia scattata alla nascita del Conflitto (FR-5, NFR-2). La ragione è il requisito, non l'implementazione: un Conflitto resta aperto per giorni, e un timestamp congelato direbbe fra tre giorni «sincronizzato alle 14:32» riferendosi a un controllo di tre giorni prima — la falsa sincronia che NFR-2 vieta, nel punto in cui costa di più, perché è il dato su cui l'Host sceglie quale Prenotazione tenere (2.7). **Dove** il dato sia memorizzato non è materia di questo AC `[DECISIONE ratificata MYL-90: opzione A — l'implementazione della 2.5 è confermata e l'AC allineato a ciò che il sistema fa]`
**And** per una Prenotazione **manuale**, che un sync non ce l'ha, la fonte è il Canale «Manuale» e il timestamp è la **data di inserimento**, con etichetta che dichiara che **non** è un dato sincronizzato — la falsa simmetria fra le due colonne della Finestra di riconciliazione (2.7) sarebbe peggio dell'asimmetria `[DECISIONE ratificata: test design Epic 2 §4.2-6]`
**And** quando la fonte è un Feed che non ha **mai** avuto un sync riuscito, il timestamp è **assente** — il sistema dice «non lo so» invece di mostrare un orario preso in prestito da qualche altra parte (NFR-2) `[registrato da MYL-90: comportamento consegnato dalla Story 2.5, non una decisione nuova; come renderlo in UI resta il punto aperto §4.2-3 del test design, di Fahad]`

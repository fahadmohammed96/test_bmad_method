...
- **Anagrafica `ospite`** (decisione di Fahad del 2026-07-26, MYL-40 → PRD §14.2, AC in `docs/epics.md` Story 2.3/2.4). Entità del modulo `calendario`, unico scrittore (AD-18), con nome ed **eventuali** contatti. Quattro regole che valgono per chiunque la tocchi: si salva **solo** ciò che arriva dal Feed o che l'Host inserisce volontariamente (campi contatto **nullable**, mai obbligatori, **mai dedotti** da testo opaco del portale); **nessun documento d'identità** fuori da `ospite_documento` (Epic 3, AD-11); **retention** come parametro configurabile legato al ciclo della Prenotazione, mai hardcodato; **tenant-owned** (`host_id` NOT NULL, mai nell'allowlist dei dati di riferimento). I dati Ospite non compaiono in log, eventi, `outbox`/`job` o notifiche.

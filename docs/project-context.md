...
- **Anagrafica `ospite`** (decisione di Fahad del 2026-07-26, MYL-40 → PRD §14.2, AC in `docs/epics.md` Story 2.3/2.4). Entità del modulo `calendario`, unico scrittore (AD-18), con nome ed **eventuali** contatti. Quattro regole che valgono per chiunque la tocchi: si salva **solo** ciò che arriva dal Feed o che l'Host inserisce volontariamente (campi contatto **nullable**, mai obbligatori, **mai dedotti** da testo opaco del portale); **nessun documento d'identità** fuori da `ospite_documento` (Epic 3, AD-11); **retention** come parametro configurabile legato al ciclo della Prenotazione, mai hardcodato, eseguita **azzerando i campi personali** e mai cancellando la riga; **tenant-owned** (`host_id` NOT NULL, mai nell'allowlist dei dati di riferimento). I dati Ospite non compaiono in log, eventi, `outbox`/`job` o notifiche. L'invariante vincolante è **AD-21** dello spine (registrato il 2026-07-26): in caso di dubbio vale l'AD, non questo riassunto.
...
### Convenzione di citazione degli ID (`AD-n`, `FR-n`, `NFR-n`, `UJ-n`)

Nelle frasi **normative** — quelle che definiscono un contratto vincolante — si cita l'**insieme**, non l'intervallo: «tutti gli invarianti `AD-n` elencati in `ARCHITECTURE-SPINE.md`» (non «AD-1…AD-21»). Un intervallo scritto a mano è **una fotografia**: resta vero solo finché nessuno aggiunge un elemento, e quando succede la frase diventa falsa senza che nulla fallisca — la voce più recente ne resta silenziosamente esclusa. È la formulazione già adottata in `docs/epics.md`.

Il numero resta ammesso **solo** dove è dichiaratamente una fotografia datata: frontmatter `depends_on`/`inputDocuments`, celle di tabelle di copertura, e le singole frasi che dichiarano un conteggio a una data — **non** l'intero documento che le contiene: anche un documento di stato, quando *punta* a un contratto vivo invece di fotografarlo, cita l'insieme. L'esenzione è della frase, non del file.
